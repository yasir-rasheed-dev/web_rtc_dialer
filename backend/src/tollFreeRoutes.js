...
async function createCampaign(req, res, ami) {
  ...
  try {
    await connection.beginTransaction();
    await connection.execute(
      `INSERT INTO inbound_campaigns (id, tenant_id, did_id, name, status, ivr_id, created_by)
       VALUES (?,?,?,?,?,?,?)`,
      [id, req.user.tenant_id, didId, name, status, ivrId, req.user.id]
    );
...
async function updateCampaign(req, res, ami) {
  ...
  try {
    await connection.beginTransaction();
    await connection.execute(
      `UPDATE inbound_campaigns SET name=?, status=?, ivr_id=? WHERE id=? AND tenant_id=?`,
      [name, status, ivrId, campaign.id, req.user.tenant_id]
    );
    if (agentIds) {
