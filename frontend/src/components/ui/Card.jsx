 ...
 */
export default function Card({ title, description, icon: Icon, actions, animate = true, className = "", children }) {
    ...
    >
      {(title || description || Icon || actions) && (
        <div className="mb-4 flex items-start justify-between gap-4">
          ...
          {actions || (Icon && <Icon size={20} className="shrink-0 text-muted" />)}
        </div>
      )}
      {children}
    </Wrapper>
