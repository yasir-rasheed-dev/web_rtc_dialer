...
import {
  ...
  Film, Music, Archive, FileSpreadsheet, Presentation,
} from "lucide-react";
...
const getChatId = (a, b) => (String(a) < String(b) ? `${a}_${b}` : `${b}_${a}`);
...
/* ─── In-App Notification ────────────────────────────── */
function InAppNotifToast({ notif, onDismiss, onOpen }) {
...
function attachTypeIcon(mimeType, fileName) {
  const mime = mimeType || "";
...
function MsgAttachView({ att, isMe }) {
...
/* ─── MsgBubbleActions ────────────────────────────────── */
function MsgBubbleActions({ isMe, msg, onEdit, onDelete, isDark }) {
